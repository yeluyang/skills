...
Refactoring tears down old structure and builds better structure. The project's existing documentation — agent-memory files such as `AGENTS.md` and `CLAUDE.md`, `README.md`, style guides, architectural decision records — **describes what the code is today, not what it should become**. Treat these documents as reference material, not as binding constraints.
...
3. **Never assume existing conventions are correct.** The conventions may themselves be the source of the code smells you are refactoring away. A style guide that mandates god-classes, an `AGENTS.md` or `CLAUDE.md` that prescribes a flat module structure, or a README that documents a tangled dependency graph — these are potentially part of the problem.
...
- It does not mean skip updating documentation. If the refactoring changes the project's architecture, remind the user that project-memory files (`AGENTS.md`, `CLAUDE.md`, and peers), `README.md`, and other docs will need corresponding updates.
